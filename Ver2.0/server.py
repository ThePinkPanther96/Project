...
try:
    ...
    sock.listen(LISTEN)
    print(Fore.WHITE + 'Listening for connections...')
...
try:
    ...
    if chan:
        ...
        while True:
            command = input(Fore.YELLOW + 'Enter a command to execute on the client (or "exit" to quit): ')
            if command.lower() == 'exit':
                break
            chan.send(command.encode('utf-8'))
            output_data = chan.recv(1024).decode('utf-8')
            print(Fore.CYAN + 'Output from client:\n', output_data)
        chan.close()
    else:
